...
One section per subsystem. Screenshots and GIFs are captured from a sanitized demonstration vault; each placeholder below marks a shot already planned.
...
> 📸 `media/home-desktop.png` — homepage with dock and verse card
> 📸 `media/home-mobile.png` — same surface on a phone
...
> 🎬 `media/select-lane.gif` — card dragged into the Select lane → draft note appears
...
> 📸 `media/media-library.png` — gallery with filters and detail pane
> 🎬 `media/capture-roundtrip.gif` — phone share → item appears in the library
...
> 📸 `media/comms-system.png` — channel registry and system console
...
> 📸 `media/people-dossier.png` — a person dossier assembled from mentions
...
> 🎬 `media/ask-select.gif` — Siri shortcut → note lands in the vault
...
> 📸 `media/finances-overview.png` — balances overview (demo data)
...
> 🎬 `media/intentions-review.gif` — dealing through a review session
...
> 📸 `media/dev-dashboard.png` — the board mid-sprint
> 📸 `media/dev-review.png` — the review room with an evidence note
