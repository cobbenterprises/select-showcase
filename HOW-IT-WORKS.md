...
About three-quarters of the plugin fleet (16 of 22) runs on the phone, and that ratio is a set of deliberate calls, not an accident of what happened to work. The question asked of every surface: **where is the human actually standing when this is useful?**
...
- **Deep work stays at the desk — on purpose.** Six surfaces are flagged desktop-only: the finance workbench (dense ledger tables and imports), document management (filesystem-heavy), the podcast pipeline, the AI-steering console, the coaching portal, and the desk-microphone voice-processing surface. Marking them desktop-only isn't neglect; it's a statement that a cramped phone rendering of a dense screen serves nobody.
